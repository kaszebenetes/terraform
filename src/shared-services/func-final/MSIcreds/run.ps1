...
$resourceURI = $Request.Query.resourceURI ?? "https://management.azure.com/";
$apiVERSION = $Request.Query.resourceURI ?? "2019-08-01";
...
$tokenAuthURI = $env:IDENTITY_ENDPOINT + "?resource=$resourceURI&api-version=$apiVERSION&client_id=$clientID"
$tokenResponse = (Invoke-WebRequest -Headers @{"X-IDENTITY-HEADER"="$env:IDENTITY_HEADER"} -Uri $tokenAuthURI).Content

# https://www.miru.ch/accessing-azure-keyvault-secrets-from-an-azure-function/
...
# TODO: Add encryption key pull from keyvault
# TODO: Add adding secret with full url+client_id to key-vault as secret/tf output
